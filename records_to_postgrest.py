...
def generate_record_id(call_number, timestamp, type):
    """
    ...
    #  compose record id from entry identifier (which is not wholly unique) and publication timestamp alongside incident type (fire or traffic).
    return f"{cf_id}_{timestamp}_{type}"
